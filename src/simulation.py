...
def qmap_statistics(
    ...
    for widx, weight_sigma in enumerate(tqdm(weight_sigmas, desc="weights")):
        ########################################################################
        # can we not separate the q_starts and t_conv calculations? would that
        # speed things up?
        ########################################################################
...
def single_layer_net_statistics(
    ...
    net = RandNet(
        din, n_hidden_units, n_hidden_layers, nonlinearity=nonlinearity,
        flip=True, dist=dist, std=std, diversity=diversity, prob_1=prob_1,
        _lambda=_lambda, init_method=init
    )
...
def multi_layer_net_statistics(
    ...
    # Flip flag here chooses whether activations or linear layer is first
    net = RandNet(
        din, n_hidden_units, n_hidden_layers-1, nonlinearity=nonlinearity,
        flip=True, dist=dist, std=std, diversity=diversity, prob_1=prob_1,
        _lambda=_lambda, init_method=init
    )
    ...
    qmaps_sim = np.zeros((nw, nb, nqnorms, n_hidden_layers, nq))
    for widx, weight_sigma in enumerate(tqdm(weight_sigmas, desc="weights")):
        for bidx, bias_sigma in enumerate(tqdm(bias_sigmas, desc="bias'")):
            net.randomize(bias_sigma=bias_sigma, weight_sigma=weight_sigma)
            for norm_idx, qnorm in enumerate(tqdm(qnorms)):
                ...
                qmaps_sim[widx, bidx, norm_idx, 1:] = 1.0 / din * (acts**2).sum(-1)
...
weight_sigmas=None, nonlinearity=None, init=None, replace=False):
    ...
    for trial in tqdm(np.arange(num_trials)):
        ...
        for widx, weight_sigma in enumerate(tqdm(weight_sigmas, desc="weights")):
            for bidx, bias_sigma in enumerate(tqdm(bias_sigmas, desc="bias'")):
                net.randomize(bias_sigma=bias_sigma, weight_sigma=weight_sigma)
                net_output = net.get_act(net_input)
...
weight_sigmas=None, nonlinearity=None, init=None, replace=False):
    ...
    for starting_correlation_index in tqdm(np.arange(n_starting_correlations)):
        ...
        for layer_index in tqdm(np.arange(n_hidden_layers)):
            ...
            correlations[starting_correlation_index, :, layer_index +
                1] = get_set_correlation(net_activations[layer_index])
